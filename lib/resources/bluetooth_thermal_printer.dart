...
class _BluetoothThermalPrinterState extends State<BluetoothThermalPrinter> {
  ...
  void _tesPrint() async {
    ...
    final _total = _format.format(widget.bill.billAmount);
    //SIZE
    // 0- normal size text
    // 1- only bold text
    ...
    bluetooth.isConnected.then((isConnected) {
      if (isConnected != null && isConnected) {
        ...
        bluetooth.printCustom(
            DateFormat.yMd().format(widget.customer.createdAt), 2, 1);
        bluetooth.printNewLine();
        ...
        bluetooth.printCustom('CU.M: ${widget.bill.consumeCM}', 1, 0);
        bluetooth.printCustom('Amount: $_total', 1, 0);
        bluetooth.printCustom('Balance: ', 1, 0);
        bluetooth.printCustom('Total: ', 1, 0);
        bluetooth.printCustom(
            'Due Date: ${DateFormat.yMd().format(widget.customer.createdAt.add(const Duration(days: 3)))}',
            1,
            0);
        bluetooth.printNewLine();
